...
static inline void digit_gen(const DiyFp W, const DiyFp Mp, uint64_t delta, char* buffer, int* len, int* K) {
    ...
    int kappa = count_decimal_digit32(p1); // kappa in [0, 9]
    *len = 0;
    ...
    while (kappa > 0) {
        ...
        if (tmp <= delta) {
            *K += kappa;
            grisu_round(buffer, *len, delta, tmp, kPow10[kappa] << -one.e, wp_w.f);
            return;
        }
    }
    ...
    for (;;) {
        ...
        if (p2 < delta) {
            *K += kappa;
            int index = -kappa;
            grisu_round(buffer, *len, delta, p2, one.f, wp_w.f * (index < 20 ? kPow10[index] : 0));
            return;
        }
    }
}
